...
async function getfile(path){
    ...
    if(file.ok){
        fdata = await file.arrayBuffer();
    }
    else return ''
    let data = await new Response(fdata).arrayBuffer()
    if(file=='')
        alert("ERROR");
...
async function getHen() {
    ...
    await sleep(150);
        ...
        if(r==0){
            ...
            msgs.tabindex = "0";
            msgs.onclick=()=>{
                display("✔成功注入GoldHen✔",1);
                msgs.style.cursor = "default";
                msgs.onclick=null;
                pl_list.style.display='block';
                pl_list3.style.display='inline-flex';
            }
        }
        else
            display("破解失败, 希望你不会断电",2);
}
